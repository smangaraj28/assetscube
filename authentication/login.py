...
def login():
    if request.method == "OPTIONS":
        ...
        site = request.headers.get("site", None)
        print(site)
        res_to_send, response = login_common(request, site)
    ...
    if s <= 0:
        if site == 'uh':
            command = cur.mogrify("""
                                    ...
                                    (select json_agg(c) from 
                                        (select cc.entityid,cc.entitybranchid,cc.entityname
                                            from unihot.entity_branch cc where entitybranchid IN (SELECT cb.entitybranch from unihot.useraccess cb where cb.userid = %s AND cb.accessstatus = 'A' AND cb.logintype = %s AND cb.usertype = %s)
                                    ...
                                    (select json_agg(d) from
                                        (select entityid,entityname from unihot.entity_branch 
                                ...
                                """, (
            userid, logintype, usertype, userid, logintype, usertype, userid, logintype, usertype, userid, logintype,
            usertype,))

            '''
            query output
            '''
            myoutput = {"userid": "p5zb1HtqemS3yrzfgIX5cKmSsrB2", "username": "natrayan p",
                        "useremail": "natrayanp@gmail.com",
                        "entitybranch": [{"entityid": "01", "entitybranchid": "01", "entityname": "PUBLIC"}],
                        "entity": [{"entityid": "01", "entityname": "PUBLIC"}],
                        "userrole": [{"entityid": "01", "entitybranchid": "01", "accessmatrix": [{"moduleid": "POS",
                                                                                                  "screens": [{
                                                                                                                  "screenid": "mainscreen",
                                                                                                                  "access": "R"},
                                                                                                              {
            ...
            command = cur.mogrify("""
                                    select row_to_json(art) from (
                                    select a.userid as uid,a.username,a.useremail
                                    from unihot.userlogin as a, unihot.useraccess as b where a.userid =  %s AND a.userstatus = 'A' AND a.logintype =  %s AND a.usertype =  %s
                                    AND a.userid = b.userid and b.site = %s) art
...
def get_user_access_details(userid, con, cur):
    command = cur.mogrify("""
                            ...
                            (select json_agg(c) from (select c.entityid,c.entitybranchid,c.entityname,(select json_agg(d) from (select entityid,entityname from unihot.entity_branch where userid = %s ) as d) as entity 
                            from unihot.entity_branch c where userid = %s ) as c) as entitybranch 
...
def signup():
    if request.method == "OPTIONS":
        ...
        respo = signup_common(sign_data, site)
        print("respo")
        print(respo)
                ...
                if site == 'uh':
                    command = cur.mogrify("""
                                WITH et AS (SELECT entityid, entitybranchid FROM unihot.entity_branch WHERE entitybranchname = 'PUBLIC'),
                                    rl AS (SELECT roleid FROM unihot.roledetails WHERE entity = (SELECT entityid FROM et) and site = %s)
                    ...
                    update_access = True
            elif logintype == "T" and usertype == "S":
                if site == 'ng':
                    print("after insert")
                    command = cur.mogrify("""
                            WITH et AS (SELECT entityid, entitybranchid FROM unihot.entity_branch WHERE entitybranchname = 'PUBLIC'),
